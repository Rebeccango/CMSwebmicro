...
(function($) {
    $.fn.extend({
        monthly: function(options) {
            ...
            $(document).arrive('#' + uniqueId + ' .monthly-event-indicator', function() {
                var id = $(this).data('eventid');
                var height = $(this).outerHeight();
                $('#' + uniqueId + ' .monthly-event-indicator[data-eventid="' + id + '"]').css("height", height);
            })

            $(document).arrive('#' + uniqueId + ' [data-toggle="popover"]', function() {

                $(this).popover({
                    html: true,
                    template: '<div class="popover monthly-popover" data-uniqueid="' + uniqueId + '" role="tooltip"><div class="popover-arrow"></div><h3 class="popover-title"></h3><div class="popover-content"></div></div>'
                });

            });

            $(document).arrive('.monthly-popover[data-uniqueid="' + uniqueId + '"]', function() {
                $(this).find('.popover-content, .popover-title').dotdotdot({
                    ellipsis: '...',
                    wrap: 'word',
            ...
            $('#' + uniqueId).prepend('<div class="monthly-header"><div class="monthly-header-title"><a href="#" class="monthly-header-title-date" onclick="return false"></a></div><a href="#" class="monthly-prev"></a><a href="#" class="monthly-next"></a></div>')
                .append('<div class="monthly-event-list"></div>')
                .append('<div class="event-page mCustomScrollbar" data-mcs-theme="dark" >\
		<div class="close-btn btn"><i class="fa fa-window-close" style="font-size: 20px;vertical-align: middle;"\ aria-hidden="true"></i></div>\
		<div class="container-fluid" style="margin-top:-25px;" >\
		<div class="event-title main-title" >\
		<h1></h1>\
		...
		<div></div>');
                ...
                if (options.mode == 'event') {
                    ...
                    var addEvents = function(event) {
                        ...
                        var fullstartDate = options.dataType == 'xml' ? $(event).find('startdate').text() : event.startdate,
                            ...
                            eventBody = options.dataType == 'xml' ? $(event).find('body').text() : event.body,

                            eventId = options.dataType == 'xml' ? $(event).find('id').text() : event.id,
                            eventType = options.dataType == 'xml' ? $(event).find('eventType').text() : event.eventType,
                            ...
                            startPeriod = 'AM',
                            endPeriod = 'PM',
                            timeframe = startMon + " " + startDay + ", " + startYear + (startTime != "" ? " @ " + startTime : "") + (endArr != "" ? " - " + endMon + " " + endDay + ", " + endYear + (endTime != "" ? " @ " + endTime : "") : ""),
                            eventBody2 = options.dataType == 'xml' ? $(event).find('body').text() : event.timeframe + '<br/>' + event.body;
                        ...
                        if (!fullendDate && startMonth == setMonth && startYear == setYear) {
                            // Add Indicators
                            $('#' + uniqueId + ' *[data-number="' + startDay + '"] .monthly-indicator-wrap').append('<div class="monthly-event-indicator" data-container="body" data-toggle="popover" data-placement="top" data-trigger="hover" data-content="' + eventBody + '" data-timeframe="' + timeframe + '" data-eventid="' + eventId + '" style="background:' + eventColor + '" title="' + eventTitle + '">' + eventTitle + '</div>').popover();
                            ...
                            for (var i = parseInt(startDay); i <= parseInt(endDay); i++) {
                                ...
                                if (i == parseInt(startDay)) {
                                    $('#' + uniqueId + ' *[data-number="' + i + '"] .monthly-indicator-wrap').append('<div  class="monthly-event-indicator" data-container="body" data-toggle="popover" data-placement="top" data-trigger="hover" data-content="' + eventBody + '" data-timeframe="' + timeframe + '" data-eventid="' + eventId + '" style="background:' + eventColor + '" title="' + eventTitle + '">' + eventTitle + '</div>');
                                } else {
                                    $('#' + uniqueId + ' *[data-number="' + i + '"] .monthly-indicator-wrap').append('<div  class="monthly-event-indicator" data-container="body" data-toggle="popover" data-placement="top" data-trigger="hover" data-content="' + eventBody + '" data-timeframe="' + timeframe + '" data-eventid="' + eventId + '" style="background:' + eventColor + '" title="' + eventTitle + '"></div>');
                                }
                                multidaylist();
                            }
            ...
            function closeEventPage(e) {
                $('#' + uniqueId + ' .event-page').css('transform', 'scale(0)');
                setTimeout(function() {
                    $('#' + uniqueId + ' .event-page').hide();
                }, 250);
            }

            $(document.body).on('click', '#' + uniqueId + ' .event-page .close-btn', function(e) {
                closeEventPage();
                e.preventDefault();
            });
            ...
            $(document.body).on('click', '#' + uniqueId + ' .monthly-event-indicator', function(e) {
                ...
                $('#' + uniqueId + ' .event-page .event-content').html($(this).data('content'));
                $('#' + uniqueId + ' .event-page .event-title h1').html($(this).data('original-title') ? $(this).data('original-title') : $(this).attr('title'));
                $('#' + uniqueId + ' .event-page .event-title .event-time').html($(this).data('timeframe'));

                $('#' + uniqueId + ' .event-page').show();
                $('#' + uniqueId + ' .event-page').css('transform');
                $('#' + uniqueId + ' .event-page').css('transform', 'scale(1)');
                $('#' + uniqueId + ' .event-page').mCustomScrollbar({
                    axis: "y" // horizontal scrollbar
                });
                $('.monthly-popover').popover('hide');

                viewToggleButton();
            });

            var clickDocument = $(document).mouseup(function(e) {
                var container = $('#' + uniqueId + ' .event-page');
                if (container.is(':visible')) {
                    ...
                    {
                        closeEventPage();
                        $(document).unbind('mouseup', clickDocument);
                    }
                    e.preventDefault();
                }
            });
